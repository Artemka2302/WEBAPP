...
def reg():
    if request.method == 'POST':
        ...
        # Проверка пароля (здесь твоя функция validate_password)
    ...
    # GET запрос
    roles = Role.query.all()
    return render_template('register.html', form_data={}, errors={}, roles=roles)

@app.route('/corrections', methods=['GET', 'POST'])
def correct():
    users = User.query.all()
    for user in users:
       print(f"User: {user.username}, created_at: {user.created_at}")  # Посмотри в консоль
